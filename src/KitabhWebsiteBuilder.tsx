...
(function(){
  ...
  if (view === "templates") {
    ...
    return (
      <div style={{ ...props.style, width: "100%", height: "100%", overflow: "auto", background: "#FAFAFA" }}>
        <style>{CSS_STYLES}</style>
        ...
        <div className="kwb-builder-body">
                      ...
                      _inner = (
                          <div className="kwb-p-header">
                            <div className="kwb-p-header-inner">
                              <div className="kwb-p-logo-wrap" style={{ cursor: "pointer" }} onClick={() => { const homePage = activeSite.pages[0]; if (homePage) { setActivePageId(homePage.id); setExpandedComponent(null); } }}>
          ...
          <div className={`kwb-sidebar${mobileBuilderSidebar ? ' kwb-sidebar-mobile-open' : ''}`}>
            ...
            <div className="kwb-sidebar-top-btns-col">
              ...
              {/* Inline template preview: first 3 templates */}
              <div style={{ marginTop: 8 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#555', fontFamily: "'IBM Plex Sans Arabic', sans-serif", marginBottom: 6, textAlign: 'right' }}>قوالب جاهزة</div>
                  ...
                  {TEMPLATES.slice(0, 3).map(tpl => {
                    ...
                    return (
                      <button
                        ...
                        title={tpl.name}
                        style={{
                          flex: 1, height: 52, border: isActive ? '2px solid #111' : '1px solid #E5E7EB',
                      ...
                      >
                        <svg viewBox="0 0 400 500" style={{ width: '100%', height: '100%' }}>
                          {tpl.id === "media" && <><rect x="0" y="200" width="400" height="100" fill={accent} opacity=".85"/><circle cx="200" cy="200" r="140" fill={btn} opacity=".6"/><circle cx="320" cy="120" r="60" fill={accent} opacity=".9"/></>}
                          {tpl.id === "newsletter" && <><path d="M0,0 L0,250 A250,250 0 0,0 250,0 Z" fill={accent} opacity=".7"/><circle cx="280" cy="350" r="160" fill="none" stroke={btn} strokeWidth="28" opacity=".6"/><circle cx="280" cy="350" r="60" fill={btn} opacity=".7"/></>}
                          {tpl.id === "blog" && <><circle cx="120" cy="150" r="150" fill={accent} opacity=".65"/><circle cx="300" cy="200" r="120" fill={btn} opacity=".55"/><circle cx="340" cy="80" r="40" fill={btn} opacity=".8"/></>}
                        </svg>
                      </button>
                    );
                  })}
...
.kwb-tpl-picker{max-width:1200px;margin:0 auto;padding:40px 32px 60px;direction:rtl;}
.kwb-tpl-picker-header{text-align:center;margin-bottom:48px;}
.kwb-tpl-picker-header .kwb-back-link{margin-bottom:16px;}
.kwb-tpl-picker-title{font-size:32px;font-weight:800;color:#1a1a1a;margin:0 0 8px;letter-spacing:-.02em;}
.kwb-tpl-picker-subtitle{font-size:15px;color:#999;margin:0;font-weight:400;}
.kwb-tpl-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:28px;}
...
.kwb-p-hero-side-img{width:100%;height:220px;object-fit:cover;display:block;}
